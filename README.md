...
2. **System status & commands** – JSON objects containing current state (broadcasting, calibrating, camera connectivity and FPS) are pushed periodically. Commands such as `save`, `reset`, `shutdown`, `start_calibration` and `stop_calibration` may be sent on this channel. Default port `8766` (see `STATUS_WEBSOCKET_PORT`).
